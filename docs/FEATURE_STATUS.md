...
> 2026-07-20（**SNSシェアカード**）：リアルイラスト単体だと「資料画像」に見えるため、達成感と進捗を添えたカードを自動生成する。`shareCard.core.ts` ＋ [ShareCardView](../src/components/dex/ShareCardView.tsx) で**6種**（単体発見 `NEW DISCOVERY` ／レア発見 `I FOUND A RARE SPECIES`（RARE以上で自動切替・発見条件を追加）／ワールド完成 `WORLD COMPLETE`（代表4体・完成率100%・完成日）／図鑑進捗 `My Collection Progress`（進捗バー・直近3体）／今日の発見 `Today's Discoveries`（1〜4体）／今週のコレクション `This Week`）。**イラストが主役**で、カードは枠と余白のみ・情報行は最大4行・イラストの上へ重ねない。配色は `dexPresentation.core` と共通のため一覧／発見演出／カードで一貫。複数体のカードは最上位分類で配色を決め、ワールド完成は常にLEGEND系。投稿文テンプレは `shareHeadlineFor()`（「レア生物を見つけた！」「ついに◯◯ワールド完成！」「SECRETを解放しました」等）を本文1行目に付与。**発見日時は日付まで丸め**（カードは生ISO文字列を保持しない）、発見地域は都道府県名まで。対象0件ならカードを出さない。**投稿されるのはテキストのみ**（画像添付は新規依存が必要なため未実装）。詳細 [COLLECTION_EXPERIENCE.md](COLLECTION_EXPERIENCE.md)。**確率・抽選・解放条件は不変**。検証：348 pass / 0 fail / TSC0。

> 2026-07-20（**レア演出仕様**）：リアルイラスト路線で弱くなりがちなレア感を、**イラストへ描き込まずUIレイヤーで**設計。`dexPresentation.core.ts` を一覧／発見演出／詳細／共有の**単一正本**に拡張し、色調を仕様どおり分離（**RARE=シルバー〜淡い虹彩**／**LEGEND=ブロンズ・ダークゴールド・アンバー**／**SECRET=深い紫・濃紺・青緑・黒金**。RAREを金系にせずLEGENDとの格差を保つ）。一覧は枠幅・カード内側の subtle glow・英字ラベル（RARE/LEGEND/SECRET、NORMALは出さない）。発見演出は入り方（fade／glow／dim_then_rise／silhouette_then_reveal）・長さ・専用SE（`discovery_legend`/`discovery_secret`/`dex_complete` を新設）・"◯ DISCOVERED" 表示・LEGEND以上の記録メタ強調・共有優先度（SECRET最優先）を定義。**再発見は常に短いフェード・再発見音・英字ラベルなし**（重い演出を繰り返さない）。詳細画面はヘッダのみ分類色（**本文の可読性は据え置き**）。完成演出は `completionCelebrationOf` / `pickCompletionCelebration` で4種（ワールド／分類／初回コンプリート／図鑑100%）を定義し、同時成立時は大きい方を1つだけ表示。ワールド完成は**代表生物を並べた記念カード**（`CompletionCelebrationCard`）。**音素材とアニメーション実装は未着手**（値は定義済み・無音で安全動作）。詳細 [COLLECTION_EXPERIENCE.md](COLLECTION_EXPERIENCE.md)。**確率・抽選・解放条件・完全秘匿仕様は不変**。検証：324 pass / 0 fail / TSC0。

> 2026-07-20（**リアルイラスト路線＋バズる体験設計**）：画像は「種の特徴が正しく分かる、見やすく整理された高品質な生物イラスト」を正とし、**超写実・マクロ写真風・標本写真・キャラクター化を禁止**。細密さで勝負せず、**一覧の統一感（サイズ感・向き・光源・描画密度）を最優先**。特別感は画像へ焼き込まず**UI側**で作る：`dexPresentation.core.ts` が分類別の枠色・バッジ・演出強度・共有強調を返し、図鑑一覧の統一カード・結果画面のヒーロー枠・共有導線が参照する。**初発見のみ演出を1段上げ、再発見は上げない**。完成率は `DexProgressBar` で可視化（**100%は全件発見時のみ＝切り捨て**、「あと1種で完成」を常時表示）。共有は `shareText.core.ts` に集約し、発見／ワールド完成／**今日の発見**／**今週のコレクション**の4導線（対象0件ならカードを出さない）。共有文面のバーコード値・ハッシュ・座標・秒時刻の混入は `findShareTextLeaks()` とテストで禁止。**虫・節足動物は専用指示** [05_ARTHROPODS.md](encyclopedia-prompts/05_ARTHROPODS.md)（口器・複眼・毛・トゲ・濡れた質感を消さずに目立たせない／標本感・ホラー感を回避／縮小表示で確認）。詳細 [COLLECTION_EXPERIENCE.md](COLLECTION_EXPERIENCE.md)。**確率・抽選・解放条件・完全秘匿仕様は不変**。検証：306 pass / 0 fail / TSC0。
