// AUTO-GENERATED by scripts/generateCharacterData.js. Do not edit by hand.
// worldGroup 構成のキャラクターメタデータ（画像requireは characterImages.generated.ts）。

/**
 * 図鑑上の分類。表示・説明文の出し分けにのみ使う。
 * NORMAL=現生生物の通常形態 / RARE=実在する希少形態（白変種等） /
 * LEGEND=絶滅生物 / SECRET=神話・伝承・空想・SF上の生物。
 * 抽選・出現確率・解放条件は従来どおり rarity を参照し、この値へは依存しない。
 */
export type DexClass = "NORMAL" | "RARE" | "LEGEND" | "SECRET";

export type CatalogCharacter = {
  id: string;
  realmGroup: string;
  worldGroup: string;
  no: number;
  name: string;
  speciesJa: string;
  speciesEn: string;
  hasImage: boolean;
  releaseStatus: string;
  /** 図鑑上の分類（表示専用）。抽選・確率・解放条件へは影響しない。 */
  dexClass: DexClass;
  status: string;
  description: string;
};

export type CatalogRare = {
  id: string;
  realmGroup: string;
  worldGroup: string;
  no: number;
  name: string;
  speciesJa: string;
  speciesEn: string;
  hasImage: boolean;
  releaseStatus: string;
  /** 図鑑上の分類（表示専用）。抽選・確率・解放条件へは影響しない。 */
  dexClass: DexClass;
  description: string;
};

export const CATALOG_CHARACTERS: CatalogCharacter[] = [
  {
    "id": "ground_aardvark",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 67,
    "name": "ツチブタン",
    "speciesJa": "ツチブタ",
    "speciesEn": "Aardvark",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_alpaca",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 1,
    "name": "モコアルパ",
    "speciesJa": "アルパカ",
    "speciesEn": "Alpaca",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_anteater",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 2,
    "name": "アリクイノ",
    "speciesJa": "アリクイ",
    "speciesEn": "Anteater",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_armadillo",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 3,
    "name": "アルマジロロ",
    "speciesJa": "アルマジロ",
    "speciesEn": "Armadillo",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_badger",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 4,
    "name": "アナグマル",
    "speciesJa": "アナグマ",
    "speciesEn": "Badger",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_bear",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 5,
    "name": "コグマル",
    "speciesJa": "クマ",
    "speciesEn": "Bear",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_bison",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 68,
    "name": "バイソン",
    "speciesJa": "バイソン",
    "speciesEn": "Bison",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_camel",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 6,
    "name": "ラクダム",
    "speciesJa": "ラクダ",
    "speciesEn": "Camel",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_cat",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 7,
    "name": "ヒカリネコ",
    "speciesJa": "猫",
    "speciesEn": "Cat",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_chameleon",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 8,
    "name": "カメレオンヌ",
    "speciesJa": "カメレオン",
    "speciesEn": "Chameleon",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_cheetah",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 9,
    "name": "チータス",
    "speciesJa": "チーター",
    "speciesEn": "Cheetah",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_chimpanzee",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 10,
    "name": "チンパル",
    "speciesJa": "チンパンジー",
    "speciesEn": "Chimpanzee",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_cow",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 11,
    "name": "ウシモー",
    "speciesJa": "牛",
    "speciesEn": "Cow",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_deer",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 12,
    "name": "コモレビシカ",
    "speciesJa": "鹿",
    "speciesEn": "Deer",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_dog",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 13,
    "name": "モリイヌ",
    "speciesJa": "犬",
    "speciesEn": "Dog",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_donkey",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 14,
    "name": "ロバロン",
    "speciesJa": "ロバ",
    "speciesEn": "Donkey",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_elephant",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 15,
    "name": "ゾウノコ",
    "speciesJa": "ゾウ",
    "speciesEn": "Elephant",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_ferret",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 16,
    "name": "フェレッタ",
    "speciesJa": "フェレット",
    "speciesEn": "Ferret",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_fox",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 17,
    "name": "ユキギツネ",
    "speciesJa": "キツネ",
    "speciesEn": "Fox",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_gecko",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 18,
    "name": "ヤモリン",
    "speciesJa": "ヤモリ",
    "speciesEn": "Gecko",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_giraffe",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 19,
    "name": "キリンネ",
    "speciesJa": "キリン",
    "speciesEn": "Giraffe",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_goat",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 20,
    "name": "ヤギノコ",
    "speciesJa": "ヤギ",
    "speciesEn": "Goat",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_gorilla",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 21,
    "name": "ゴリマル",
    "speciesJa": "ゴリラ",
    "speciesEn": "Gorilla",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_hamster",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 22,
    "name": "ハナハム",
    "speciesJa": "ハムスター",
    "speciesEn": "Hamster",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_hedgehog",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 23,
    "name": "ハリネズン",
    "speciesJa": "ハリネズミ",
    "speciesEn": "Hedgehog",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_horse",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 24,
    "name": "ウマカゼ",
    "speciesJa": "馬",
    "speciesEn": "Horse",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_hyena",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 25,
    "name": "ハイエニャ",
    "speciesJa": "ハイエナ",
    "speciesEn": "Hyena",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_iguana",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 26,
    "name": "イグアロン",
    "speciesJa": "イグアナ",
    "speciesEn": "Iguana",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_jaguar",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 27,
    "name": "ジャガロン",
    "speciesJa": "ジャガー",
    "speciesEn": "Jaguar",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_kangaroo",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 28,
    "name": "カンガルン",
    "speciesJa": "カンガルー",
    "speciesEn": "Kangaroo",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_koala",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 29,
    "name": "コアラン",
    "speciesJa": "コアラ",
    "speciesEn": "Koala",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_lemur",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 30,
    "name": "キツネザルン",
    "speciesJa": "キツネザル",
    "speciesEn": "Lemur",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_leopard",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 31,
    "name": "ヒョウテン",
    "speciesJa": "ヒョウ",
    "speciesEn": "Leopard",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_lion",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 32,
    "name": "コライオン",
    "speciesJa": "ライオン",
    "speciesEn": "Lion",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_lizard",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 33,
    "name": "リクトカゲ",
    "speciesJa": "トカゲ",
    "speciesEn": "Lizard",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_llama",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 34,
    "name": "ラマロン",
    "speciesJa": "ラマ",
    "speciesEn": "Llama",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_mandrill",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 35,
    "name": "マンドリルン",
    "speciesJa": "マンドリル",
    "speciesEn": "Mandrill",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_marmot",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 36,
    "name": "マーモル",
    "speciesJa": "マーモット",
    "speciesEn": "Marmot",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_marten",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 37,
    "name": "テンテン",
    "speciesJa": "テン",
    "speciesEn": "Marten",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_meerkat",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 38,
    "name": "ミーアミィ",
    "speciesJa": "ミーアキャット",
    "speciesEn": "Meerkat",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_mole",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 39,
    "name": "モグリット",
    "speciesJa": "モグラ",
    "speciesEn": "Mole",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_monkey",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 40,
    "name": "サルッコ",
    "speciesJa": "サル",
    "speciesEn": "Monkey",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_moose",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 41,
    "name": "ムースン",
    "speciesJa": "ムース",
    "speciesEn": "Moose",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_mouse",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 42,
    "name": "ドングリネズ",
    "speciesJa": "ネズミ",
    "speciesEn": "Mouse",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_okapi",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 43,
    "name": "オカピン",
    "speciesJa": "オカピ",
    "speciesEn": "Okapi",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_orangutan",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 44,
    "name": "オランゴ",
    "speciesJa": "オランウータン",
    "speciesEn": "Orangutan",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_panda",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 45,
    "name": "パンダル",
    "speciesJa": "パンダ",
    "speciesEn": "Panda",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_pangolin",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 66,
    "name": "センザンコ",
    "speciesJa": "センザンコウ",
    "speciesEn": "Pangolin",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_porcupine",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 46,
    "name": "ヤマアラシン",
    "speciesJa": "ヤマアラシ",
    "speciesEn": "Porcupine",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_prairie_dog",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 47,
    "name": "プレリード",
    "speciesJa": "プレーリードッグ",
    "speciesEn": "Prairie Dog",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_rabbit",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 48,
    "name": "ツキウサ",
    "speciesJa": "うさぎ",
    "speciesEn": "Rabbit",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_raccoon",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 49,
    "name": "ホシアライ",
    "speciesJa": "アライグマ",
    "speciesEn": "Raccoon",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_raccoon_dog",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 50,
    "name": "マメタヌキ",
    "speciesJa": "タヌキ",
    "speciesEn": "Raccoon Dog",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_red_panda",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 51,
    "name": "レッサン",
    "speciesJa": "レッサーパンダ",
    "speciesEn": "Red Panda",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_reindeer",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 69,
    "name": "トナカイン",
    "speciesJa": "トナカイ",
    "speciesEn": "Reindeer",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_rhinoceros",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 52,
    "name": "サイガン",
    "speciesJa": "サイ",
    "speciesEn": "Rhinoceros",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_serow",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 53,
    "name": "カモシカゼ",
    "speciesJa": "カモシカ",
    "speciesEn": "Serow",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_sheep",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 54,
    "name": "ツノヒツジ",
    "speciesJa": "羊",
    "speciesEn": "Sheep",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_skunk",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 55,
    "name": "スカンクー",
    "speciesJa": "スカンク",
    "speciesEn": "Skunk",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_sloth",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 56,
    "name": "ナマケロン",
    "speciesJa": "ナマケモノ",
    "speciesEn": "Sloth",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_snake",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 57,
    "name": "ヘビマキ",
    "speciesJa": "ヘビ",
    "speciesEn": "Snake",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_squirrel",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 58,
    "name": "コノハリス",
    "speciesJa": "リス",
    "speciesEn": "Squirrel",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_tapir",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 59,
    "name": "バクミィ",
    "speciesJa": "バク",
    "speciesEn": "Tapir",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_tiger",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 60,
    "name": "トラマル",
    "speciesJa": "トラ",
    "speciesEn": "Tiger",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_vicuna",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 61,
    "name": "ビクーニャル",
    "speciesJa": "ビクーニャ",
    "speciesEn": "Vicuna",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_weasel",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 62,
    "name": "イタチル",
    "speciesJa": "イタチ",
    "speciesEn": "Weasel",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_wild_boar",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 63,
    "name": "イノマル",
    "speciesJa": "イノシシ",
    "speciesEn": "Wild Boar",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_wolf",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 64,
    "name": "オオカミン",
    "speciesJa": "オオカミ",
    "speciesEn": "Wolf",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "ground_zebra",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 65,
    "name": "シマウマル",
    "speciesJa": "シマウマ",
    "speciesEn": "Zebra",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "sky_bat",
    "realmGroup": "life",
    "worldGroup": "sky",
    "no": 1,
    "name": "コウモリン",
    "speciesJa": "コウモリ",
    "speciesEn": "Bat",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "sky_cockatoo",
    "realmGroup": "life",
    "worldGroup": "sky",
    "no": 2,
    "name": "オウムン",
    "speciesJa": "オウム",
    "speciesEn": "Cockatoo",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "sky_crane",
    "realmGroup": "life",
    "worldGroup": "sky",
    "no": 3,
    "name": "ツルミヤ",
    "speciesJa": "ツル",
    "speciesEn": "Crane",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "sky_crow",
    "realmGroup": "life",
    "worldGroup": "sky",
    "no": 4,
    "name": "カラスケ",
    "speciesJa": "カラス",
    "speciesEn": "Crow",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "sky_eagle",
    "realmGroup": "life",
    "worldGroup": "sky",
    "no": 5,
    "name": "ワシノスケ",
    "speciesJa": "ワシ",
    "speciesEn": "Eagle",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "○"
  },
  {
    "id": "sky_flamingo",
    "realmGroup": "life",
    "worldGroup": "sky",
    "no": 6,
    "name": "フラミン",
    "speciesJa": "フラミンゴ",
    "speciesEn": "Flamingo",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "11"
  },
  {
    "id": "sky_hawk",
    "realmGroup": "life",
    "worldGroup": "sky",
    "no": 7,
    "name": "タカマル",
    "speciesJa": "タカ",
    "speciesEn": "Hawk",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "11"
  },
  {
    "id": "sky_ostrich",
    "realmGroup": "life",
    "worldGroup": "sky",
    "no": 8,
    "name": "ダチョラン",
    "speciesJa": "ダチョウ",
    "speciesEn": "Ostrich",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "11"
  },
  {
    "id": "sky_owl",
    "realmGroup": "life",
    "worldGroup": "sky",
    "no": 9,
    "name": "フクロウル",
    "speciesJa": "フクロウ",
    "speciesEn": "Owl",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "11"
  },
  {
    "id": "sky_parakeet",
    "realmGroup": "life",
    "worldGroup": "sky",
    "no": 10,
    "name": "インコロ",
    "speciesJa": "インコ",
    "speciesEn": "Parakeet",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "11"
  },
  {
    "id": "sky_peacock",
    "realmGroup": "life",
    "worldGroup": "sky",
    "no": 11,
    "name": "クジャックン",
    "speciesJa": "クジャク",
    "speciesEn": "Peacock",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "11"
  },
  {
    "id": "sky_pigeon",
    "realmGroup": "life",
    "worldGroup": "sky",
    "no": 12,
    "name": "ハトポッポ",
    "speciesJa": "ハト",
    "speciesEn": "Pigeon",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "11"
  },
  {
    "id": "sky_sparrow",
    "realmGroup": "life",
    "worldGroup": "sky",
    "no": 13,
    "name": "スズメル",
    "speciesJa": "スズメ",
    "speciesEn": "Sparrow",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "11"
  },
  {
    "id": "sky_swallow",
    "realmGroup": "life",
    "worldGroup": "sky",
    "no": 14,
    "name": "ツバメロ",
    "speciesJa": "ツバメ",
    "speciesEn": "Swallow",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "11"
  },
  {
    "id": "sky_swan",
    "realmGroup": "life",
    "worldGroup": "sky",
    "no": 15,
    "name": "ハクチョル",
    "speciesJa": "白鳥",
    "speciesEn": "Swan",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "NORMAL",
    "description": "11",
    "status": "11"
  }
];

export const CATALOG_NORMALS: CatalogCharacter[] = CATALOG_CHARACTERS;

export const CATALOG_RARES: CatalogRare[] = [
  {
    "id": "ground_rare_white_tiger",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 2,
    "name": "白虎",
    "speciesJa": "白虎",
    "speciesEn": "White Tiger",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "RARE",
    "description": "11"
  }
];

export const CATALOG_LEGENDARIES: CatalogRare[] = [
  {
    "id": "ground_rare_fenrir",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 1,
    "name": "フェンリル",
    "speciesJa": "フェンリル",
    "speciesEn": "Fenrir",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "SECRET",
    "description": "11"
  },
  {
    "id": "ground_rare_tsuchinoko",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 3,
    "name": "ツチノコ",
    "speciesJa": "ツチノコ",
    "speciesEn": "Tsuchinoko",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "SECRET",
    "description": "11"
  },
  {
    "id": "ground_rare_underground_dweller",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 5,
    "name": "地底人",
    "speciesJa": "地底人",
    "speciesEn": "Underground Dweller",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "SECRET",
    "description": "11"
  },
  {
    "id": "ground_rare_yeti",
    "realmGroup": "life",
    "worldGroup": "ground",
    "no": 4,
    "name": "イエティ",
    "speciesJa": "イエティ",
    "speciesEn": "Yeti",
    "hasImage": true,
    "releaseStatus": "initial",
    "dexClass": "SECRET",
    "description": "11"
  }
];
